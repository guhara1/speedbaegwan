...
# 실제 시공 사진 (구글 드라이브 제공분, webp) — 캡션은 사진 내용에 맞춤
GALLERY = [
    ("누수탐지 현장", "leak-detection"), ("욕실 배관 누수 시공", "bathroom-leak"),
    ("주방 배관 시공", "kitchen-leak"), ("누수탐지·압력 점검", "meter-leak"),
    ("변기·욕실 시공", "toilet-clog"), ("싱크대 배관 시공", "sink-clog"),
    ("세면대·수전 시공", "basin-clog"), ("하수구 역류·맨홀 점검", "drain-backflow"),
    ("하수구 고압세척", "high-pressure"), ("관로 내시경 진단", "endoscope"),
    ("전문 시공 장비", "kitchen-faucet"), ("관로 내시경 점검", "bath-faucet"),
    ("옥외 배수구 시공", "toilet-replace"), ("수도 계량기 점검", "basin-replace"),
    ("노후 배관 점검", "toilet-parts"), ("노후 배관 교체 시공", "pipe-replace"),
    ("상가 배관 설비 시공", "commercial"), ("배관 설비 시공", "factory"),
    ("옥외 하수관 시공", "emergency"), ("관로 내시경 장비 점검", "inspection"),
]
...
GUIDES = [
    {
        "slug": "junggan-valve-anjamgim", "cat": "설비", "service": "seolbi", "date": "2026-08-06",
        "img": "guide-junggan-valve-anjamgim",
        "title": "물을 잠갔는데 안 멈추거나 밸브에서 샐 때 — 중간밸브(앵글밸브) 문제와 대처",
        "desc": "수전·변기 수리의 첫 관문은 중간밸브 잠그기입니다. 안 잠기거나 뻑뻑하거나 축에서 샐 때의 상황별 대처와, 억지로 돌리면 안 되는 이유·교체 순서를 정리했습니다.",
        "lead": "오래 안 돌린 밸브는 안 잠기거나 새기 쉽습니다. 억지로 힘주면 부러져 물이 뿜어지니, 상황별 대처를 알고 시작하세요.",
        "html": """
<h2>중간밸브 위치부터</h2>
<ul>
  <li>세면대·싱크대: 수납장 안 호스 아래 은색 밸브(온수·냉수)</li>
  <li>변기: 뒤쪽·탱크 아래 호스 연결부</li>
</ul>
<p>손잡이는 <b>시계 방향 끝까지=잠김</b>입니다.</p>
<h2>상황별 대처</h2>
<ul>
  <li><b>잠가도 물이 나옴</b>: 패킹 노후 → <b>세대 메인</b>으로 차단, 밸브 교체</li>
  <li><b>뻑뻑해 안 돌아감</b>: 억지로 X(부러지면 분출) → 메인 잠그고 살살</li>
  <li><b>축에서 샘</b>: 패킹너트를 <b>1/8바퀴만</b> 살짝 조임, 그래도 새면 교체</li>
</ul>
<h2>교체 순서</h2>
<p>메인부터 잠금 → 물·압력 빼기 → 기존 밸브 풀기 → 나사에 <b>생테이프 6~8회</b> 감아 새 밸브 → 물 열어 누수 확인.</p>
""",
        "faq": [
            ("중간밸브를 잠갔는데도 물이 계속 나와요.", "밸브 내부 패킹이 오래돼 완전히 막히지 않는 상태입니다. 이 밸브만 믿지 말고 세대 메인 밸브로 물을 차단한 뒤 작업하세요. 근본적으로는 그 중간밸브를 교체해야 하며, 교체할 때도 메인부터 잠근 뒤 진행합니다."),
            ("밸브가 뻑뻑한데 힘줘서 돌려도 되나요?", "안 됩니다. 몇 년 안 돌린 밸브는 굳어 있어 억지로 힘주면 축이 부러지며 물이 뿜어질 수 있습니다. 세대 메인 밸브를 먼저 잠근 뒤 살살 돌려보고, 그래도 안 되거나 벽 배관에 고착·부식됐으면 무리하지 말고 점검을 받으세요."),
        ],
    },
    {
        "slug": "yoksil-silicone-gompangi", "cat": "설비", "service": "seolbi", "date": "2026-08-05",
        "img": "guide-yoksil-silicone-gompangi",
        "title": "욕실 실리콘에 곰팡이가 슬고 벌어질 때 — 방치하면 안 되는 이유와 재시공",
        "desc": "욕조·세면대 이음새 실리콘 곰팡이·벌어짐은 미관이 아니라 방수 문제입니다. 청소로 될 때와 재시공할 때 구분법, 덧바르면 안 되는 이유와 셀프 재시공 순서를 정리했습니다.",
        "lead": "실리콘은 물이 벽 뒤로 새지 않게 막는 마지막 방어선입니다. 벌어진 틈으로 스민 물은 방수층·아랫집까지 번집니다.",
        "html": """
<h2>미관이 아니라 방수 문제</h2>
<p>이음새 실리콘이 벌어지면 그 틈으로 물이 <b>벽·바닥 뒤로 스며</b> 방수층 손상·벽 뒤 곰팡이·아랫집 누수로 번집니다.</p>
<h2>청소 vs 재시공</h2>
<ul>
  <li>표면 곰팡이만(실리콘 멀쩡) → <b>청소</b></li>
  <li>벽에서 들뜸·벌어짐·갈라짐, 속까지 검음 → <b>재시공</b></li>
</ul>
<h2>곰팡이 청소</h2>
<p>제거제·락스 묻힌 휴지를 <b>덮어 30분~1시간</b> 밀착(뿌리기만 하면 약함) → 헹구고 환기. 다른 세제와 섞지 않기.</p>
<h2>재시공은 '걷어내고 말린 뒤'</h2>
<p>기존 실리콘 위에 덧바르면 안쪽 곰팡이·습기가 갇힙니다. 완전 제거 → 완전 건조 → 방균 욕실용 실리콘 → 24시간 물 금지.</p>
""",
        "faq": [
            ("낡은 실리콘 위에 새로 덧발라도 되나요?", "안 됩니다. 겉만 깨끗해 보일 뿐 안쪽 곰팡이와 습기가 그대로 갇혀 금방 다시 검게 올라오고, 벌어진 틈의 물길도 막지 못합니다. 기존 실리콘을 완전히 걷어내고 속까지 바싹 말린 뒤 방균 욕실용 실리콘으로 새로 시공해야 합니다."),
            ("실리콘만 다시 하면 누수가 잡히나요?", "표면 이음새 문제면 재시공으로 해결되지만, 걷어냈을 때 벽·타일 뒤가 젖어 있거나 아래층에 얼룩이 있으면 이미 물이 벽 뒤로 샌 것이라 방수층까지 봐야 합니다. 타일 들뜸·줄눈 파손이 넓으면 방수 시공이 필요하니 점검을 받으세요."),
        ],
    },
    {
        "slug": "byeongi-neomchim-eunggeup", "cat": "막힘", "service": "makhim", "date": "2026-08-04",
        "img": "guide-byeongi-neomchim-eunggeup",
        "title": "변기 물이 넘칠 것처럼 차오를 때 — 당황하지 않고 넘침 막는 법",
        "desc": "막힌 변기가 차오를 때 레버를 또 누르면 그대로 넘칩니다. 탱크 마개·급수 밸브로 넘침을 멈추는 응급 순서와 밸브 위치를 미리 알아두는 법을 정리했습니다.",
        "lead": "차오르는 몇 초엔 뚫는 것보다 '멈추는 것'이 먼저입니다. 레버를 다시 누르지 않고 물부터 막으면 침수를 피할 수 있습니다.",
        "html": """
<h2>레버를 다시 누르지 마세요</h2>
<p>물이 안 빠지는데 한 번 더 누르면 탱크 물이 또 쏟아져 넘칩니다. 뚫기 전에 <b>물부터 멈추는 것</b>이 먼저입니다.</p>
<h2>넘치기 직전 10초</h2>
<ol>
  <li>레버에서 손 떼기(다시 누르지 않기)</li>
  <li>탱크 뚜껑 열어 <b>고무마개(플래퍼)를 손으로 눌러 닫기</b></li>
  <li>탱크 볼탑을 잡거나 <b>변기 급수 밸브를 잠그기</b></li>
</ol>
<h2>급수 밸브 위치</h2>
<p>변기 뒤쪽·물탱크 아래 <b>은색 작은 밸브</b>를 시계방향으로 잠그면 물이 안 들어옵니다. 안 되면 세대 메인 밸브를 잠급니다.</p>
<p>멈춘 뒤 물을 절반쯤 덜어내고, 물을 남긴 채 뚫어뻥으로 배수구를 덮어 눌렀다 당기기를 반복합니다.</p>
""",
        "faq": [
            ("변기가 차오르는데 어떻게 멈추나요?", "레버를 다시 누르지 말고, 탱크 뚜껑을 열어 고무마개(플래퍼)를 손으로 눌러 닫아 변기로 가는 물을 막으세요. 이어 탱크 볼탑을 잡거나 변기 뒤쪽·아래의 급수 밸브를 시계방향으로 잠그면 더 이상 차오르지 않습니다. 급수 밸브가 안 되면 세대 메인 밸브를 잠급니다."),
            ("물티슈를 빠뜨려 막혔는데 뚫어뻥으로 될까요?", "물티슈·기저귀·생리대·수건은 물에 풀리지 않아 뚫어뻥으로 밀면 더 깊이 박힐 수 있습니다. 무리하게 밀지 말고 꺼내야 합니다. 여러 번 시도해도 계속 차오르거나 욕실 바닥 배수구까지 함께 역류하면 공용 배수관 문제일 수 있어 점검을 받는 것이 안전합니다."),
        ],
    },
    {
        "slug": "sudonmul-naemsae-mat", "cat": "설비", "service": "seolbi", "date": "2026-08-03",
        "img": "guide-sudonmul-naemsae-mat",
        "title": "수돗물에서 이상한 냄새·맛이 날 때 — 원인 구분과 대처",
        "desc": "소독약·흙·비린내 등 물맛 이상은 종류에 따라 원인이 다릅니다. 냄새 종류로 원인 좁히는 법, 흘려보내기로 되는 것과 배관·물탱크 점검이 필요한 것을 정리했습니다.",
        "lead": "물맛·냄새 이상은 종류부터 가려야 합니다. 잠깐 흘려보내면 되는 것도, 우리 집 배관·물탱크 문제인 것도 있습니다.",
        "html": """
<h2>냄새 종류로 원인 좁히기</h2>
<ul>
  <li><b>소독약(락스) 냄새</b>: 염소 소독 냄새. 받아두거나 끓이면 옅어짐</li>
  <li><b>흙·곰팡이 냄새</b>: 여름철 상수원 문제일 수 있음(동네 전체·대개 무해)</li>
  <li><b>비린내·쇠 맛 + 붉은 물</b>: 노후 배관·물탱크 녹·침전물</li>
  <li><b>온수에서만</b>: 온수기·보일러·온수 배관</li>
</ul>
<h2>2분 자가 확인</h2>
<ol>
  <li>찬물/온수 비교 → 온수만이면 온수기 쪽</li>
  <li>아침 첫 물 vs 틀어둔 물 → 첫 물만이면 정체수</li>
  <li>우리 집만 vs 동네 전체 → 배관/물탱크 vs 상수원</li>
</ol>
<p>붉은 물·쇠 맛이 계속되면 배관 점검, 아파트면 저수조 청소 여부를 확인하세요.</p>
""",
        "faq": [
            ("수돗물에서 소독약 냄새가 나는데 마셔도 되나요?", "수돗물 소독에 쓰는 염소 냄새로, 오히려 소독이 되고 있다는 뜻입니다. 냄새가 거슬리면 물을 받아 뚜껑 없이 잠시 두거나 끓이면 염소가 날아가 옅어집니다. 다만 비린내·쇠 맛이거나 물색이 붉으면 이건 소독 냄새가 아니라 배관·물탱크 문제일 수 있어 확인이 필요합니다."),
            ("아침 첫 물에서만 맛이 이상해요.", "밤새 관에 정체됐던 물 때문일 가능성이 큽니다. 아침 첫 물을 몇십 초 흘려보낸 뒤 쓰면 대부분 좋아집니다. 흘려보내도 계속 이상하거나 붉은 기·이물질이 보이면 노후 배관·물탱크 쪽을 점검하고, 아파트라면 저수조 청소 여부를 관리주체에 확인해 보세요."),
        ],
    },
    {
        "slug": "byeok-mulsori-sumeun-nusu", "cat": "누수", "service": "nusu", "date": "2026-08-02",
        "img": "guide-byeok-mulsori-sumeun-nusu",
        "title": "밤에 벽에서 물소리가 들릴 때 — 안 보이는 누수 찾는 법",
        "desc": "아무도 물을 안 쓰는데 벽·바닥에서 물소리가 나면 어딘가 계속 새고 있다는 신호입니다. 계량기로 진짜 누수인지 5분 확인하는 법과 위치 좁히는 단서를 정리했습니다.",
        "lead": "압력이 걸린 급수관이 새면 물이 좁은 틈으로 뿜어져 물소리가 납니다. 벽 속이라 안 보여 방치되기 쉬운 만큼, 조기 확인이 피해를 줄입니다.",
        "html": """
<h2>안 쓰는데 물소리가 난다면</h2>
<p>수도를 다 잠갔는데도 벽·바닥에서 물소리가 계속되면 <b>급수관 누수</b> 신호입니다. 눈에 안 보여 방치되면 요금 폭탄·부식·아랫집 피해로 커집니다.</p>
<h2>진짜 누수인지 5분 확인</h2>
<ol>
  <li>집 안 모든 물(수도·세탁기·변기 급수) 잠그기</li>
  <li><b>계량기의 은색 별 표시</b>가 도는지 확인 → 돌면 누수</li>
  <li>숫자 적어두고 2~3시간 뒤 늘었는지 확인</li>
</ol>
<h2>위치 좁히는 단서</h2>
<ul>
  <li>소리 센 벽에 귀/드라이버 대고 밤에 추적</li>
  <li>특정 벽만 따뜻함 → 온수관 누수</li>
  <li>벽지 들뜸·얼룩·곰팡이 위치</li>
</ul>
<p>벽 속 정확한 지점은 누수탐지 장비로 최소한만 뜯어 잡는 것이 가장 저렴합니다.</p>
""",
        "faq": [
            ("물을 다 잠갔는데 계량기가 조금씩 돌아요.", "어딘가에서 물이 새고 있다는 뜻입니다. 수도꼭지·세탁기·변기 급수까지 모두 잠근 상태에서 계량기의 은색 별 표시가 돌거나, 2~3시간 뒤 숫자가 늘었다면 누수가 확실합니다. 벽 속은 눈으로 위치를 잡기 어려워 누수탐지로 최소한만 뜯어 잡는 것이 정확합니다."),
            ("벽이 따뜻하면서 물소리가 나요.", "온수관 누수일 가능성이 큽니다. 보일러·온수기 쪽 밸브를 잠가 온수 라인 압력을 낮췄을 때 소리가 멎으면 온수관이 유력합니다. 새는 위치의 젖은 자국·계량기 수치를 기록해 두면 점검과 보험 처리에 도움이 됩니다."),
        ],
    },
    {
        "slug": "byeongi-badak-nusu-gasket", "cat": "누수", "service": "nusu", "date": "2026-08-01",
        "img": "guide-byeongi-badak-nusu-gasket",
        "title": "변기 밑에서 물이 새고 냄새가 날 때 — 바닥 개스킷 문제",
        "desc": "변기 바닥 틈에서 물이 비치고 하수 냄새가 계속 난다면 탱크가 아니라 변기 밑 개스킷 밀봉이 샙니다. 실리콘만 다시 쏘면 안 되는 이유와 자가 확인법을 정리했습니다.",
        "lead": "변기 '밑'에서 새는 건 탱크 누수와 다른 문제입니다. 바닥 틈에 실리콘만 다시 쏘면 오히려 물을 가둬 악화시킵니다.",
        "html": """
<h2>탱크가 아니라 '바닥 개스킷'</h2>
<p>변기와 바닥 배수구를 잇는 <b>개스킷(밀봉 링)</b>이 굳거나 변기가 흔들려 밀봉이 깨지면, 물 내릴 때마다 틈으로 오수·하수 가스가 샙니다.</p>
<h2>이 증상이면 의심</h2>
<ul>
  <li>물 내린 직후에만 바닥 틈이 젖음(탱크는 상시)</li>
  <li>청소·환기해도 <b>하수 냄새</b>가 계속</li>
  <li>변기가 좌우로 <b>흔들림</b>, 주변 타일 변색·아래층 얼룩</li>
</ul>
<h2>자가 확인</h2>
<ol>
  <li>바닥 물기 닦고 세게 내린 직후 젖는지 확인</li>
  <li>변기 밀어 흔들림 확인</li>
  <li>변기 물에 식용색소 풀고 내려, 새는 물에 색이 있으면 오수 누수</li>
</ol>
<p>바닥 틈에 실리콘만 다시 쏘면 새는 물이 갇혀 더 썩습니다. 근본은 <b>개스킷 교체</b>입니다.</p>
""",
        "faq": [
            ("변기 바닥 틈에 실리콘을 다시 쏘면 되나요?", "겉은 깔끔해 보여도 새는 원인은 그대로인데 물이 빠져나올 길만 막아 바닥 아래에 물이 고입니다. 결국 바닥이 더 썩고 아랫집 피해가 커집니다. 실리콘은 앞·옆만 마감하고 뒤쪽은 조금 터놓는 것이 정석이며, 근본 해결은 개스킷 교체입니다."),
            ("변기가 흔들리는데 그냥 둬도 되나요?", "흔들림이 개스킷 밀봉을 계속 망가뜨려 누수와 냄새의 원인이 됩니다. 고정 볼트가 풀렸을 수도, 이미 밀봉이 깨졌을 수도 있어 방치하면 바닥·아래층까지 번집니다. 흔들리면 원인을 확인하고 밀봉을 다시 잡는 것이 좋습니다."),
        ],
    },
    {
        "slug": "baesugu-kkureukkeuk-tonggi", "cat": "막힘", "service": "makhim", "date": "2026-07-31",
        "img": "guide-baesugu-kkureukkeuk-tonggi",
        "title": "배수구에서 '꾸르륵' 소리가 날 때 — 통기·봉수 문제 진단법",
        "desc": "물 빠질 때 나는 꾸르륵 소리는 관 속 공기 흐름 문제 신호입니다. 소리로 원인을 좁히는 법, 완전히 막히기 전 집에서 먼저 해볼 것을 정리했습니다.",
        "lead": "그 소리는 소음이 아니라 관이 보내는 전조입니다. 대개 완전히 막히기 전 단계라, 초기에 잡으면 큰 공사를 피할 수 있습니다.",
        "html": """
<h2>'꾸르륵' 소리는 공기 흐름 문제 신호</h2>
<p>물이 내려갈 때 공기가 못 드나들면 물이 봉수를 빨아당겨 소리가 납니다. <b>막힘·냄새의 공통 전조</b>입니다.</p>
<h2>소리로 원인 좁히기</h2>
<ul>
  <li>한 배수구만 느리게 빠지며 꾸르륵 → 그 아래 <b>부분 막힘</b></li>
  <li>변기 내릴 때 옆 배수구서 소리·거품 → <b>공용관·통기</b> 문제</li>
  <li>안 쓰는데 꿀렁+냄새 → <b>봉수가 빨려 내려감</b></li>
</ul>
<h2>먼저 해볼 것</h2>
<ol>
  <li>거름망·봉수통 분리해 안쪽 청소</li>
  <li>배수구에 물 부어 봉수 채우고 다시 확인</li>
</ol>
<p>여러 배수구에서 동시에 나면 공용 배수·통기관 점검이 정확합니다.</p>
""",
        "faq": [
            ("변기를 내리면 옆 세면대·바닥 배수구에서 소리가 나요.", "여러 기구가 만나는 공용 배수관이나 통기(공기) 통로에 문제가 있을 수 있습니다. 한 곳만의 막힘이 아니라 공용관 쪽 신호일 가능성이 크니, 배수구 청소로 해결되지 않으면 관 내부·통기구 점검을 받는 것이 정확합니다."),
            ("소리만 나고 물은 잘 빠지는데 괜찮나요?", "지금은 부분 막힘·봉수 부족 단계일 수 있습니다. 방치하면 완전 막힘이나 상습 냄새 역류로 이어지므로, 거름망 청소와 봉수 채우기를 먼저 해보고 반복되면 점검해 두는 편이 안전합니다."),
        ],
    },
    {
        "slug": "syawogi-suapyak-seokhoe", "cat": "설비", "service": "seolbi", "date": "2026-07-31",
        "img": "guide-syawogi-suapyak-seokhoe",
        "title": "샤워기 물줄기가 약하고 갈라질 때 — 수압 아닌 석회질, 셀프 청소법",
        "desc": "특정 샤워기만 물이 약하면 수압이 아니라 헤드·필터에 낀 석회질 때문입니다. 원인 가려내는 법과 부품 교체 없이 물줄기 살리는 청소 순서를 정리했습니다.",
        "lead": "집 전체가 아니라 그 샤워기만 약하다면 수압 문제가 아니라 그 기구가 막힌 것입니다. 10분이면 직접 뚫을 수 있습니다.",
        "html": """
<h2>수압 탓이 아닐 수 있습니다</h2>
<p><b>그 샤워기만 약하고 다른 수도는 정상</b>이라면 수압이 아니라 헤드·필터에 낀 <b>석회질·이물질</b>입니다.</p>
<h2>원인 가려내기</h2>
<ul>
  <li>이 샤워기만 약함 → 석회질(셀프 청소로 해결)</li>
  <li>온수만 약함 → 각도밸브·카트리지·온수기</li>
  <li>집 전체가 약함 → 수압·공용 배관 점검</li>
</ul>
<h2>셀프 청소</h2>
<ol>
  <li>헤드 분리 → <b>식초·구연산에 30분~1시간</b> 담그기</li>
  <li>노즐 구멍을 칫솔로 문질러 물때 제거</li>
  <li>연결부 <b>거름망(필터)</b> 빼서 모래·녹 헹구기</li>
</ol>
""",
        "faq": [
            ("샤워헤드를 청소해도 그대로예요.", "수전 끝 거품기(에어레이터)나 아래 각도밸브를 확인하세요. 거품기도 돌려 빼서 망을 청소하면 물줄기가 살아나고, 각도밸브가 반쯤 잠겨 있으면 열어줍니다. 그래도 안 되면 카트리지·급수 쪽 문제일 수 있습니다."),
            ("청소해도 자꾸 다시 약해져요.", "물에 석회 성분이 많은 환경이라 물때가 잘 낍니다. 주기적 청소가 필요하거나 필터 설치를 고려할 만합니다. 다만 집 안 모든 수도가 함께 약하거나 갑자기 뚝 떨어졌다면 기구가 아닌 감압밸브·공용 배관 문제이니 점검을 받으세요."),
        ],
    },
    {
        "slug": "nusu-gyeollo-gubun", "cat": "누수", "service": "nusu", "date": "2026-07-31",
        "img": "guide-nusu-gyeollo-gubun",
        "title": "벽·바닥이 젖는데 누수인지 결로인지 헷갈릴 때 — 구분법",
        "desc": "결로를 누수로 오인해 멀쩡한 배관을 뜯는 경우가 많습니다. 젖는 시간·모양·물 사용 연관으로 집에서 방향을 잡는 구분법과 원인별 대처를 정리했습니다.",
        "lead": "새지도 않는 벽을 뜯어 돈만 쓰는 일을 피하려면, 벽을 열기 전에 누수인지 결로인지부터 가려야 합니다.",
        "html": """
<h2>뜯기 전에 누수인지 결로인지</h2>
<p>배관은 멀쩡하고 <b>결로</b>가 원인인 경우가 많습니다. 구분 없이 벽부터 뜯으면 돈만 씁니다.</p>
<h2>언제 젖는가</h2>
<ul>
  <li><b>결로</b>: 춥고 환기 안 될 때(밤·겨울·북향), 환기하면 마름</li>
  <li><b>누수</b>: 날씨와 무관하게 늘 젖고, 물 쓰면 심해짐</li>
  <li><b>빗물</b>: 비 올 때만 젖음(외벽·방수 문제)</li>
</ul>
<h2>모양으로 구분</h2>
<ul>
  <li>결로: 넓게 골고루 축축 + 이슬 맺힘, 차가운 벽</li>
  <li>누수: 한 지점서 시작해 흘러내린 얼룩·뚝뚝</li>
</ul>
<p>환기·제습으로 마르면 결로, 계속 젖으면 누수탐지가 정확합니다.</p>
""",
        "faq": [
            ("결로랑 누수를 어떻게 구분하나요?", "젖는 시점이 가장 큰 단서입니다. 춥고 환기 안 될 때 넓게 이슬처럼 맺히고 환기하면 마르면 결로, 날씨와 무관하게 늘 젖고 물을 쓴 뒤 심해지며 한 지점에서 배어 나오면 누수입니다. 며칠 제습해 확연히 마르면 결로 비중이 큽니다."),
            ("결로면 배관 공사를 해야 하나요?", "아닙니다. 결로는 배관 문제가 아니라 환기·제습·단열의 문제입니다. 창을 자주 열고 찬 벽에 가구를 붙이지 않으며, 심하면 단열·결로 방지 시공으로 잡습니다. 배관을 뜯을 일이 아니니 먼저 방향부터 정확히 잡으세요."),
        ],
    },
    {
        "slug": "singkeudae-gireumtae-gwanri", "cat": "막힘", "service": "makhim", "date": "2026-07-31",
        "img": "guide-singkeudae-gireumtae-gwanri",
        "title": "싱크대가 자꾸 막히는 진짜 이유 — 기름때, 이렇게 관리하세요",
        "desc": "주방 막힘의 진짜 원인은 음식물이 아니라 관 안쪽에 굳는 기름입니다. 뜨거운 물이 오히려 문제인 이유, 기름 버리는 법, 다시 안 막히게 하는 습관을 정리했습니다.",
        "lead": "뚫어도 몇 달 뒤 또 막히는 건 기름이 관 안쪽에 굳어 쌓이기 때문입니다. 이건 뚫는 문제가 아니라 안 쌓이게 하는 문제입니다.",
        "html": """
<h2>주방 막힘의 진짜 원인은 '기름'</h2>
<p>관을 갈라보면 안쪽에 <b>굳은 기름층</b>이 혈관 막히듯 눌어붙어 있습니다. 뚫어도 또 막히는 이유입니다.</p>
<h2>뜨거운 물로 흘려도 괜찮다는 건 오해</h2>
<p>녹은 기름은 <b>찬 배수관 벽을 만나 다시 굳습니다.</b> 없애는 게 아니라 안쪽으로 옮기는 것뿐입니다.</p>
<h2>기름 버리는 습관</h2>
<ul>
  <li>팬 기름은 굳혀서 종량제로, 절대 싱크대에 붓지 않기</li>
  <li>설거지 전 <b>키친타월로 기름을 한 번 닦아내고</b> 씻기</li>
  <li>국물은 식혀 뜬 기름을 걷어 버리기</li>
</ul>
<p>반복해 막히면 관 지름이 좁아진 상태라, <b>고압세척으로 기름층을 벗겨내는</b> 것이 확실합니다.</p>
""",
        "faq": [
            ("뚫어도 몇 달 뒤 또 막혀요.", "관 안쪽에 기름이 두껍게 굳어 관 지름이 좁아진 상태입니다. 약품이나 뚫어뻥은 잠깐 뚫릴 뿐 근본이 안 잡힙니다. 고압세척·스프링 장비로 눌어붙은 기름층을 벗겨낸 뒤, 기름을 하수구로 안 보내는 습관을 들이면 재발이 크게 줍니다."),
            ("베이킹소다와 식초로 기름때가 풀리나요?", "냄새나 얇게 낀 유분에는 도움이 되지만, 두껍게 굳은 기름층을 뚫는 힘은 없습니다. 물이 졸졸 빠지거나 꾸르륵 소리가 나기 시작하면 이미 상당히 좁아진 신호이니 관 청소를 고려하세요."),
        ],
    },
    {
        "slug": "setakgi-geubsu-hose", "cat": "누수", "service": "nusu", "date": "2026-07-31",
        "img": "guide-setakgi-geubsu-hose",
        "title": "세탁기 급수 호스, 터지기 전에 바꾸세요 — 침수 사고 막는 점검법",
        "desc": "외출 중 집이 잠기는 사고의 흔한 원인이 세탁기 급수 호스입니다. 24시간 수압을 받는 이 호스의 위험 신호, 교체 주기, 침수를 막는 습관을 정리했습니다.",
        "lead": "급수 호스는 수도가 열려 있는 한 24시간 수압을 받습니다. 그래서 낡으면 사람 없는 밤이나 외출 중에 터져 피해가 커집니다.",
        "html": """
<h2>왜 하필 급수 호스인가</h2>
<p>배수 호스와 달리 급수 호스는 <b>밸브가 열려 있는 한 늘 수압을 받습니다.</b> 낡으면 사람 없을 때 터져 피해가 큽니다.</p>
<h2>지금 확인할 위험 신호</h2>
<ul>
  <li>겉면 잔금·갈라짐, 한쪽이 볼록하게 부풀음</li>
  <li>연결부에 물때·석회·녹물 자국(미세 누수)</li>
  <li>설치 5년 이상 · 한 번도 안 바꿈</li>
</ul>
<h2>예방</h2>
<ul>
  <li><b>교체 주기 3~5년</b>, 스테인리스 브레이드·오토스톱 호스 권장</li>
  <li><b>세탁 후·외출 전 수도꼭지 잠그기</b> — 가장 확실한 예방</li>
</ul>
""",
        "faq": [
            ("호스가 겉으론 멀쩡한데 꼭 바꿔야 하나요?", "고무는 겉이 멀쩡해도 안에서부터 삭습니다. 급수 호스는 24시간 수압을 받는 소모품이라 3~5년마다 교체가 안전합니다. 몇 만 원 호스를 미루다 수백만 원 침수·아랫집 배상으로 이어지는 경우가 많습니다."),
            ("외출할 때마다 수도를 잠그기 번거로워요.", "적어도 여행·장기 외출·취침 전에는 잠그시길 권합니다. 사람이 없을 때가 가장 위험하기 때문입니다. 번거로우면 호스가 터지면 스스로 잠기는 오토스톱 급수 호스로 바꾸는 것도 좋은 대안입니다."),
        ],
    },
    {
        "slug": "janggi-buje-sudo", "cat": "설비", "service": "seolbi", "date": "2026-07-31",
        "img": "guide-janggi-buje-sudo",
        "title": "며칠 집 비울 때, 수도 이렇게 하고 가세요 — 부재 중 배관 체크리스트",
        "desc": "누수 사고는 방치 시간이 피해를 정합니다. 여행·출장 등 장기 부재 전 수도 잠그는 법, 돌아왔을 때 나는 하수구 냄새(봉수 마름)와 겨울 동파 대비를 정리했습니다.",
        "lead": "사고는 집에 아무도 없을 때 가장 크게 번집니다. 떠나기 전 10분이면 부재 중 침수·냄새·동파를 대부분 막을 수 있습니다.",
        "html": """
<h2>떠나기 전 체크리스트</h2>
<ol>
  <li><b>세대 메인 수도 밸브를 잠그기(가장 확실)</b></li>
  <li>어렵다면 세탁기·식기세척기·정수기 급수 밸브만이라도 잠그기</li>
  <li>보일러·가스는 외출 모드 또는 차단</li>
</ol>
<h2>돌아왔을 때 냄새 — 봉수 마름</h2>
<p>오래 물을 안 쓰면 배수구 봉수가 <b>증발</b>해 하수 냄새가 올라옵니다. 고장이 아닙니다.</p>
<ul>
  <li>떠나기 전 배수구에 물을 채우고, 오래 비우면 랩으로 덮기</li>
  <li>돌아와 냄새 나면 배수구에 물을 부어 봉수를 다시 채우기</li>
</ul>
<h2>겨울이라면 동파 대비</h2>
<p>난방을 완전히 끄지 말고 <b>보일러 외출(저온) 모드</b>, 노출 배관은 보온재로 감싸둡니다.</p>
""",
        "faq": [
            ("오래 집을 비웠더니 하수구 냄새가 심해요.", "배관 고장이 아니라 봉수(배수구에 고인 물)가 증발해서 냄새 통로가 열린 것입니다. 여름·장기 부재일수록 심합니다. 화장실·주방·베란다 배수구에 물을 충분히 부어 봉수를 다시 채우면 대부분 사라집니다."),
            ("장기 여행 때 수도 밸브를 꼭 잠가야 하나요?", "잠그는 것이 가장 안전합니다. 세대 메인 밸브를 잠그면 안에서 무엇이 터져도 물이 안 나와 침수를 원천 차단합니다. 어렵다면 세탁기·식기세척기·정수기 급수 밸브만이라도 잠그세요. 이 연결부들이 부재 중 사고의 단골 원인입니다."),
        ],
    },
    {
        "slug": "arutjip-nusu-daecheo", "cat": "누수", "service": "nusu", "date": "2026-07-30",
        "img": "guide-arutjip-nusu-daecheo",
        "title": "아랫집에서 누수 연락이 왔을 때 — 책임 확인과 대처 순서",
        "desc": "천장 누수라고 무조건 윗집 책임은 아닙니다. 그 자리에서 인정하면 안 되는 이유, 원인 규명·보험·복구까지 손해 안 보는 순서를 정리했습니다.",
        "lead": "당황해서 먼저 사과부터 하면 배상 책임을 전부 떠안기 쉽습니다. 원인이 확정돼야 누구 책임인지 정해집니다. 순서가 중요합니다.",
        "html": """
<h2>그 자리에서 책임을 인정하지 마세요</h2>
<p>아직 어디서·무엇 때문에 새는지 모르는 상태입니다. 사과도 부인도 아닌 <b>"같이 원인부터 확인하자"</b>가 정답입니다.</p>
<h2>항상 윗집 책임은 아닙니다</h2>
<ul>
  <li>윗집 전유부(세대 안) 배관·기구</li>
  <li>여러 세대가 쓰는 <b>공용배관</b> → 관리주체 몫일 수 있음</li>
  <li>외벽 균열·방수 불량·결로 등 <b>아랫집 자체</b> 문제</li>
</ul>
<h2>대처 순서</h2>
<ol>
  <li>같이 현장 확인 + 사진·영상 기록</li>
  <li><b>누수탐지</b>로 원인부터 규명(어느 관인지)</li>
  <li>관리사무소 접수, <b>일상생활배상책임보험</b> 등 특약 확인</li>
  <li>복구·배상 합의는 원인 확정 후</li>
</ol>
""",
        "faq": [
            ("천장 누수면 무조건 윗집 책임인가요?", "아닙니다. 원인이 공용배관이면 관리주체, 외벽·방수·결로면 아랫집 자체 문제일 수 있습니다. 누수탐지로 원인을 규명해야 책임 주체가 정해집니다. 원인 확인 전에 배상을 합의하지 마세요."),
            ("누수 보험이 있는지 어떻게 확인하나요?", "실손·자동차·주택보험에 '일상생활배상책임보험' 특약이 붙어 있는 경우가 많습니다. 가족 중 누구 명의로든 하나 있으면 쓸 수 있는 경우가 흔하니 온 가족 증권을 확인하고, 사고접수부터 해두세요. 접수는 책임 인정과 다릅니다."),
        ],
    },
    {
        "slug": "bangbang-pari-hasugu", "cat": "막힘", "service": "makhim", "date": "2026-07-30",
        "img": "guide-bangbang-pari-hasugu",
        "title": "화장실에 작은 날벌레가 계속 생길 때 — 하수구 나방파리 없애는 법",
        "desc": "화장실 벽에 붙는 작은 벌레는 초파리가 아니라 하수구에서 번식하는 나방파리입니다. 약을 뿌려도 안 없어지는 이유와 근본 청소법을 정리했습니다.",
        "lead": "음식물을 치워도 안 없어지는 건 이 벌레가 하수구 안쪽 슬라임에서 번식하기 때문입니다. 잡는 게 아니라 번식지를 없애야 합니다.",
        "html": """
<h2>초파리가 아니라 '나방파리'</h2>
<p>하트 모양 날개의 작은 벌레는 <b>하수구·배수구 안에서 번식</b>하는 나방파리입니다. 음식물을 치워도 안 사라지는 이유입니다.</p>
<h2>약을 뿌려도 다시 생기는 이유</h2>
<p>살충제는 보이는 성충만 잡고, 배수구 안쪽 <b>슬라임 속 알·애벌레</b>는 그대로라 계속 부화합니다. 번식지를 없애야 합니다.</p>
<h2>번식지 청소 순서</h2>
<ol>
  <li>배수구 뚜껑·거름망·봉수통을 분리</li>
  <li>솔로 관 입구 안쪽 <b>미끈한 막을 박박 긁어냄</b></li>
  <li>따뜻한 물로 헹구고 베이킹소다+식초로 마무리</li>
  <li>세면대는 P트랩까지 분리 청소</li>
</ol>
""",
        "faq": [
            ("살충제를 뿌려도 며칠 뒤 또 생겨요.", "성충만 잡혔고 배수구 안쪽 슬라임에 알·애벌레가 남아 계속 부화하기 때문입니다. 배수구를 열어 안쪽 미끈한 막을 솔로 긁어내는 '번식지 청소'를 해야 근본적으로 없어집니다."),
            ("청소해도 여러 곳에서 계속 나와요.", "더 깊은 관이나 공용배관 안쪽에 번식지가 있을 수 있습니다. 여러 배수구에서 동시에 나오면 개별 청소로는 한계가 있어, 고압세척으로 관 내부를 씻어내는 것이 확실합니다."),
        ],
    },
    {
        "slug": "semyeondae-ptrap-nusu", "cat": "설비", "service": "seolbi", "date": "2026-07-30",
        "img": "guide-semyeondae-ptrap-nusu",
        "title": "세면대 아래에서 물이 샐 때 — P트랩 누수 직접 잡는 법",
        "desc": "세면대 밑 누수는 대부분 벽 속이 아니라 P트랩 연결부에서 샙니다. 새는 지점 찾는 법과 부품 교체 없이 직접 잡는 순서를 정리했습니다.",
        "lead": "세면대 아래가 젖으면 벽부터 뜯을 필요 없습니다. 가장 흔한 범인은 눈에 보이는 P트랩이고, 10분이면 직접 확인할 수 있습니다.",
        "html": """
<h2>벽부터 뜯지 마세요</h2>
<p>세면대 아래 누수는 대부분 보이는 <b>P트랩(U자관) 연결부</b>에서 샙니다. 벽 공사가 아니라 직접 잡히는 경우가 많습니다.</p>
<h2>새는 지점 찾기</h2>
<ol>
  <li>트랩·이음부를 마른 휴지로 감쌈</li>
  <li>물을 가득 받았다 <b>한 번에 내려</b> 어느 휴지가 젖는지 확인</li>
</ol>
<h2>흔한 원인과 처리</h2>
<ul>
  <li>연결 너트 풀림 → 손으로 시계방향으로 조임</li>
  <li>고무 패킹 어긋남·노후 → 바로 끼우거나 같은 규격 교체</li>
  <li>트랩 균열 → 규격 맞춰 트랩 교체</li>
  <li>이물질로 막혀 역류 → 트랩 열어 청소</li>
</ul>
""",
        "faq": [
            ("공구로 세게 조여도 되나요?", "플라스틱 너트는 공구로 세게 조이면 금이 가 오히려 더 샙니다. 손힘으로 시계방향으로 조이는 정도면 충분하고, 안 풀리는 오래된 것만 공구를 살짝 쓰세요. 조립 후 물 테스트로 마른 상태를 확인합니다."),
            ("트랩은 말랐는데 계속 젖어요.", "벽 속 배수관이나 급수관 누수일 수 있습니다. 눈에 안 보여 누수탐지가 정확하고, 수납장 바닥·벽이 부풀거나 곰팡이가 심하면 오래 샌 것이라 마감 복구까지 필요할 수 있습니다."),
        ],
    },
    {
...
""",
    ...
    {
        "slug": "bide-mulsaem-jueui", "cat": "교체", "service": "gyoche", "date": "2025-09-24",
        "title": "비데에서 물이 샐 때 — 연결부 점검과 조치",
        "desc": "비데를 설치했거나 오래 썼더니 바닥에 물이 고일 때, 새는 위치별 원인과 집에서 할 수 있는 조치를 정리했습니다.",
        "lead": "비데 물샘은 대부분 급수 연결부(T밸브·호스)나 패킹 문제입니다. 새는 위치를 보면 원인을 좁힐 수 있습니다.",
        "html": """
<h2>새는 위치로 원인 좁히기</h2>
<ul>
  <li><b>급수호스 연결부</b>(분기 T밸브~비데) 풀림 또는 패킹 노후</li>
  <li><b>분기 T밸브와 벽 수도</b> 연결부</li>
  <li><b>비데 본체 하부</b> — 제품 노후·결함일 수 있음</li>
</ul>
<h2>집에서 할 수 있는 조치</h2>
<ol>
  <li>먼저 <b>각지 밸브(수도 잠금)</b>를 잠급니다. 그냥 분리하면 물이 쏟아질 수 있습니다.</li>
  <li>연결부를 다시 조이고, 필요하면 <b>테프론테이프</b>를 감아 조립합니다.</li>
  <li>연결부 <b>패킹(오링)</b>이 상했으면 같은 규격으로 교체합니다.</li>
  <li>본체 하부에서 계속 새면 제품 자체 문제일 수 있어 A/S를 확인합니다.</li>
</ol>
""",
        "faq": [
            ("설치하자마자 물이 새요.", "연결부 조임 불량이나 패킹 눌림이 대부분입니다. 각지 밸브를 잠근 뒤 분리해 다시 조립하면 해결되는 경우가 많습니다."),
            ("벽 수도 쪽에서 새는 것 같아요.", "벽 급수관·매립부 누수라면 타일·벽체 작업이 필요할 수 있어 전문 점검을 권합니다."),
        ],
    },
    {
        "slug": "boiler-apryeok-nanbang-nusu", "cat": "설비", "service": "seolbi", "date": "2025-09-10",
        "title": "보일러 압력이 자꾸 떨어질 때 — 난방 배관 누수 신호",
        "desc": "보일러 압력계가 계속 떨어지고 물을 보충해도 또 낮아진다면 난방수 누수를 의심해야 합니다. 확인 방법을 정리했습니다.",
        "lead": "보일러 압력이 반복해서 떨어진다면 난방수가 어딘가로 새고 있을 수 있습니다. 순서대로 확인해 보세요.",
        "html": """
<h2>누수를 의심하는 신호</h2>
<ul>
  <li>압력계가 <b>정상 범위(대개 1~2bar)</b> 아래로 자주 내려감</li>
  <li>보충밸브로 채워도 <b>하루이틀 만에 다시 하락</b>이 반복됨</li>
  <li>특정 방·구역만 <b>난방이 약하거나</b> 공기 소리가 남</li>
</ul>
<h2>점검 포인트</h2>
<ol>
  <li>보일러 주변 <b>배관 연결부</b>에 물자국·부식이 있는지 봅니다.</li>
  <li>각 방 밸브가 모인 <b>분배기</b> 주변을 확인합니다.</li>
  <li>바닥 매립 난방 배관은 육안으로 찾기 어려워 <b>누수탐지</b>가 필요합니다.</li>
</ol>
""",
        "faq": [
            ("그냥 물을 계속 보충하면 안 되나요?", "임시방편일 뿐 근본 해결이 아닙니다. 계속 새면 난방 효율이 떨어지고 바닥·구조 손상으로 이어질 수 있어 원인 수리가 필요합니다."),
            ("바닥을 다 뜯어야 하나요?", "먼저 누수탐지로 새는 지점을 좁히면 해당 부위만 최소로 열어 시공할 수 있습니다."),
        ],
    },
    {
        "slug": "jangma-hasugu-yeongnyu-yebang", "cat": "막힘", "service": "makhim", "date": "2025-06-26",
        "title": "장마철 하수구 역류 예방과 대처",
        "desc": "집중호우 때 하수구·배수구가 역류하는 이유와, 장마 전 미리 대비하는 법을 정리했습니다.",
        "lead": "장마철 역류는 배관에 물이 몰리거나 이물질로 배수가 느려서 생깁니다. 미리 손봐두면 피해를 크게 줄일 수 있습니다.",
        "html": """
<h2>장마 전 미리 점검</h2>
<ul>
  <li>배수구·맨홀 <b>거름망 이물질</b>을 제거하고, 잘 안 내려가던 곳은 미리 뚫어둡니다.</li>
  <li>마당·옥상·베란다 배수구의 <b>낙엽·흙·모래</b>를 걷어냅니다.</li>
  <li>역류가 잦은 집은 <b>역류방지밸브</b> 설치를 검토합니다.</li>
</ul>
<h2>역류가 시작되면</h2>
<ol>
  <li>저지대·반지하는 <b>물막이·양수기</b>를 준비합니다.</li>
  <li>변기·배수구로 물이 올라오면 사용을 자제합니다.</li>
  <li>물이 찰 때는 <b>감전 위험</b>이 있으니 전기 콘센트·차단기를 주의합니다.</li>
</ol>
""",
        "faq": [
            ("매년 장마마다 같은 곳이 역류해요.", "배관 구배나 공용 배관 용량 문제일 수 있습니다. 개별 청소로 한계가 있으면 전문 점검과 고압세척, 역류방지밸브를 검토하세요."),
            ("역류방지밸브를 달면 완전히 막히나요?", "역류를 크게 줄여주지만 설치 위치·상태 관리가 중요합니다. 현장에 맞는 방식은 점검 후 안내받는 것이 좋습니다."),
        ],
    },
    {
        "slug": "oksang-beranda-bitmul-nusu", "cat": "누수", "service": "nusu", "date": "2025-06-12",
        "title": "옥상·베란다·외벽에서 물이 샐 때 — 빗물 누수 구분법",
        "desc": "비 올 때만 천장·벽이 젖는다면 배관이 아니라 방수·외벽 문제일 수 있습니다. 원인 구분과 대처를 정리했습니다.",
        "lead": "누수라고 다 배관 문제는 아닙니다. '언제 젖는지'를 보면 배관 누수인지 빗물 누수인지 구분할 수 있습니다.",
        "html": """
<h2>먼저 구분하기</h2>
<ul>
  <li><b>비 올 때·비 온 뒤에만</b> 젖음 → 방수층·외벽 크랙으로 <b>빗물</b>이 스미는 경우가 많습니다.</li>
  <li><b>날씨와 무관하게 상시</b> 젖음 → <b>배관 누수</b> 가능성이 큽니다.</li>
</ul>
<h2>빗물 누수의 흔한 원인</h2>
<ul>
  <li>옥상 <b>방수층 노후·균열</b></li>
  <li>베란다·옥상 <b>배수구 막힘</b>으로 물이 고임</li>
  <li>외벽 <b>크랙</b>·창틀 실리콘 노후</li>
</ul>
<h2>대처</h2>
<ol>
  <li>옥상·베란다 <b>배수구를 청소</b>하고 방수 상태를 점검합니다.</li>
  <li>배관인지 빗물인지 애매하면 <b>누수탐지</b>로 원인을 구분합니다.</li>
</ol>
""",
        "faq": [
            ("비 올 때만 천장이 젖어요.", "빗물 누수(방수·크랙) 가능성이 높습니다. 다만 배관 누수와 겹치는 경우도 있어 애매하면 탐지로 확인하는 것이 정확합니다."),
            ("배관 누수인지 빗물인지 구분이 안 돼요.", "젖는 시점(날씨와의 관계)과 위치를 보고, 누수탐지 장비로 배관 쪽 누수 여부를 확인하면 명확해집니다."),
        ],
    },
    {
        "slug": "aircon-baesu-makhim", "cat": "막힘", "service": "makhim", "date": "2026-07-22",
        "title": "에어컨에서 물이 뚝뚝 떨어질 때 — 드레인(배수) 호스 막힘",
        "desc": "여름철 에어컨 실내기에서 물이 새는 원인은 대개 냉방 고장이 아니라 배수 호스 막힘입니다. 원인과 집에서 할 수 있는 대처를 정리했습니다.",
        "lead": "에어컨 물이 새면 '고장'부터 걱정하지만, 실제로는 물이 빠지는 드레인 호스가 막힌 경우가 대부분입니다.",
        "html": """
<h2>에어컨 물은 원래 어디로 빠지나</h2>
<p>실내기에 맺힌 물은 물받이에 모였다가 <b>드레인 호스</b>를 타고 실외로 빠집니다. 이 통로가 막히면 물이 실내기 앞으로 넘쳐 뚝뚝 떨어집니다.</p>
<h2>흔한 원인</h2>
<ul>
  <li>호스 안쪽 <b>곰팡이·슬라임</b> 축적</li>
  <li>호스 <b>처짐·꺾임</b>으로 물 고임</li>
  <li>호스 끝 <b>벌레·먼지</b>로 막힘</li>
</ul>
<h2>집에서 해볼 것</h2>
<ol>
  <li>전원을 끄고 물받이 물을 닦아냅니다.</li>
  <li>실외 호스 끝의 이물질을 제거하고 꺾인 곳을 정리합니다.</li>
  <li>그래도 계속 새면 물받이·안쪽 배관 막힘이므로 점검을 받습니다.</li>
</ol>
""",
        "faq": [
            ("호스 끝은 뚫었는데 계속 물이 새요.", "물받이나 실내기 안쪽 배관이 막힌 경우가 많습니다. 억지로 하면 내부가 젖을 수 있어 물받이 분리 세척과 호스 내부 청소를 받는 편이 안전합니다."),
            ("에어컨 물 새는 걸 예방하려면요?", "필터를 자주 청소하고, 냉방을 끄기 전 송풍 모드로 내부를 말리면 곰팡이·슬라임이 덜 껴 배수 막힘이 줄어듭니다."),
        ],
    },
    {
        "slug": "baegwan-sueum-sugyeok", "cat": "설비", "service": "seolbi", "date": "2026-07-15",
        "title": "수도 잠글 때 '쾅' 소리(수격현상)가 날 때 — 원인과 해결",
        "desc": "수도나 세탁기 밸브를 잠글 때 벽에서 '쾅' 소리와 진동이 난다면 수격현상입니다. 방치하면 누수로 이어지는 이유와 해결법을 정리했습니다.",
        "lead": "밸브를 잠그는 순간 나는 '쾅' 소리는 수격현상(워터해머)입니다. 반복되면 이음부가 헐거워져 누수로 이어질 수 있습니다.",
        "html": """
<h2>왜 소리가 나나</h2>
<p>빠르게 흐르던 물이 밸브가 갑자기 닫히며 멈출 곳을 잃고 관 벽을 때리는 충격입니다. 세탁기·식기세척기처럼 밸브가 순간적으로 닫히는 기기에서 특히 심합니다.</p>
<h2>줄이는 방법</h2>
<ul>
  <li>꼭지를 <b>천천히 잠그기</b></li>
  <li>수압이 너무 높으면 <b>감압밸브</b>로 조정</li>
  <li>덜렁거리는 노출 배관을 <b>고정</b></li>
  <li>세탁기·비데 쪽엔 <b>수격방지기</b> 설치</li>
</ul>
""",
        "faq": [
            ("수격방지기를 꼭 달아야 하나요?", "세탁기·식기세척기·비데처럼 전자밸브가 스스로 순식간에 닫히는 기기는 사람이 조절할 수 없어, 수격방지기를 다는 것이 근본 해결에 가깝습니다."),
            ("소리를 그냥 두면 어떻게 되나요?", "충격이 매번 반복되면 이음부가 헐거워지고 패킹이 삭아 누수로 이어질 수 있습니다. 소리가 커지거나 물자국이 함께 보이면 점검을 권합니다."),
        ],
    },
    {
        "slug": "hwajangsil-yuga-yeongnyu", "cat": "막힘", "service": "makhim", "date": "2026-07-08",
        "title": "화장실 바닥 배수구에서 물이 안 빠지거나 역류할 때 — 유가 막힘",
        "desc": "샤워물이 안 빠지거나 변기 물이 바닥 배수구로 올라온다면 원인 위치가 다릅니다. 유가 청소법과 역류 대처를 정리했습니다.",
        "lead": "바닥 배수구(유가)는 그 배수구만 느린 '국소 막힘'과, 다른 곳 물이 올라오는 '역류'의 원인 위치가 다릅니다.",
        "html": """
<h2>증상부터 구분</h2>
<ul>
  <li><b>그 배수구만 느림</b> → 머리카락·비누때 국소 막힘(직접 청소로 해결)</li>
  <li><b>변기·세면대 쓰면 바닥으로 역류</b> → 아래 공용 배관 막힘 신호</li>
</ul>
<h2>국소 막힘 청소</h2>
<ol>
  <li>뚜껑·거름망을 들어내 머리카락 뭉치를 건져냅니다.</li>
  <li>봉수통을 씻고 베이킹소다·식초로 정리한 뒤 원래 순서대로 끼웁니다.</li>
</ol>
<h2>역류라면</h2>
<p>물 사용을 멈추고, 관선·내시경으로 막힌 지점을 확인해야 합니다. 청소만으로는 끝나지 않습니다.</p>
""",
        "faq": [
            ("변기 물을 내리면 바닥 배수구로 물이 올라와요.", "그 배수구가 아니라 아래 공용 배관(횡주관)이 막혔다는 신호입니다. 바닥 청소로는 해결되지 않으니 물 사용을 멈추고 배관 점검을 받으세요."),
            ("배수구를 청소해도 냄새가 나요.", "봉수가 말랐거나 봉수통이 빠진 경우가 많습니다. 안 쓰는 배수구에도 가끔 물을 흘려 봉수를 채우고, 봉수통이 파손됐으면 교체하세요."),
        ],
    },
    {
        "slug": "eumsigmul-cheorigi-baegwan", "cat": "막힘", "service": "makhim", "date": "2026-07-25",
        "title": "음식물처리기(디스포저) 설치 전 알아야 할 배관 상식 — 막힘·규정",
        "desc": "음식물처리기를 달면 편리하지만 잘못 쓰면 싱크대 배관이 막힙니다. 종류·막힘 원인·설치 전 확인할 규정을 정리했습니다.",
        "lead": "음식물처리기는 편리한 만큼 배관 관리가 필요합니다. 갈린 찌꺼기가 관에 쌓이는 구조를 알면 막힘을 예방할 수 있습니다.",
        "html": """
<h2>종류부터 다릅니다</h2>
<p>분쇄해 하수로 흘리는 <b>디스포저</b>, 갈아 모으는 거름형, 열로 말리는 건조형이 있습니다. 배관과 직접 얽히는 건 디스포저입니다.</p>
<h2>왜 막히나</h2>
<ul>
  <li>갈린 찌꺼기가 꺾인 관·완만한 경사에 다시 <b>쌓임</b></li>
  <li><b>기름</b>은 갈아도 식으면 굳어 관 벽에 달라붙음</li>
  <li>물을 적게 틀고 갈면 찌꺼기가 관을 못 넘어감</li>
</ul>
<h2>안 막히게 쓰는 법</h2>
<ol>
  <li>갈 때 <b>찬물을 충분히</b> 틀고, 다 갈린 뒤에도 10~20초 더 흘립니다.</li>
  <li>기름·뼈·질긴 섬유질은 넣지 않습니다.</li>
  <li>지역 <b>하수 규정</b> 허용 여부와 배관 상태를 설치 전 확인합니다.</li>
</ol>
""",
        "faq": [
            ("디스포저를 달았는데 자꾸 막혀요.", "갈린 찌꺼기와 굳은 기름이 아래 배관에 쌓인 경우가 많습니다. 약품은 표면만 녹이므로, 관을 열어 제거하거나 고압세척으로 관 안을 씻어내야 재발을 막습니다."),
            ("아무 집에나 설치할 수 있나요?", "지역마다 하수 직배출 허용 기준이 다르고, 오래됐거나 경사가 부족한 배관은 찌꺼기가 쌓여 맞지 않을 수 있습니다. 설치 전 규정과 배관 상태를 먼저 확인하세요."),
        ],
    },
    {
        "slug": "jeongsugi-jiksu-nusu", "cat": "누수", "service": "nusu", "date": "2026-07-24",
        "title": "정수기·냉장고 직수 연결부 누수 — 안 보이는 곳에서 새는 물 찾기",
        "desc": "정수기·냉장고 직수 호스 연결부는 눈에 안 띄는 곳에 있어 누수를 늦게 발견합니다. 잘 새는 지점과 집에서 확인하는 법을 정리했습니다.",
        "lead": "누수라고 다 배관 본체 문제는 아닙니다. 정수기·냉장고 직수 연결부에서 조금씩 새다 밑판·바닥이 상해서야 발견되는 경우가 많습니다.",
        "html": """
<h2>어디서 잘 새나</h2>
<ul>
  <li>수도 <b>분기 밸브(T밸브)</b> 패킹·조임</li>
  <li>정수기·냉장고 쪽 <b>플라스틱 피팅</b> 균열</li>
  <li>가는 <b>직수 호스</b>의 눌림·꺾임</li>
  <li>필터 교체 시 <b>오링</b> 어긋남</li>
</ul>
<h2>집에서 확인</h2>
<ol>
  <li>싱크대 아래·냉장고 뒤 바닥에 <b>마른 휴지</b>를 깔아 젖는 지점을 찾습니다.</li>
  <li>연결부마다 휴지로 감싸 어느 쪽이 젖는지 봅니다.</li>
  <li>확인 중엔 급수 밸브를 잠가 피해를 막습니다.</li>
</ol>
""",
        "faq": [
            ("연결부를 조였는데 계속 물방울이 맺혀요.", "플라스틱 피팅은 과하게 조이면 갈라지니 세게 조이지 마세요. 조여도 계속 맺히거나 금이 보이면 테이프로 감지 말고 부품을 교체하는 것이 맞습니다."),
            ("연결부는 말랐는데 바닥이 계속 젖어요.", "배관 본체 누수일 가능성이 있습니다. 휴지 테스트로도 지점이 안 잡히면 미세 누수라 누수탐지 장비로 확인하는 편이 정확합니다."),
        ],
    },
    {
        "slug": "nohu-baegwan-nokmul", "cat": "설비", "service": "seolbi", "date": "2026-07-23",
        "title": "수도에서 녹물이 나올 때 — 노후 배관 신호와 교체 시기",
        "desc": "아침에 붉은 녹물이 나오거나 수압이 떨어졌다면 노후 배관 신호입니다. 교체 시기 판단과 전체·부분 수리 차이를 정리했습니다.",
        "lead": "녹물은 보기 안 좋은 문제를 넘어, 배관 수명이 얼마 남지 않았다는 가장 분명한 경고입니다.",
        "html": """
<h2>노후 배관의 신호</h2>
<ul>
  <li>아침 첫 물에 <b>붉은 녹물</b>(계속 붉으면 부식 진행)</li>
  <li>예전보다 약해진 <b>수압</b></li>
  <li>같은 집에서 <b>반복되는 누수</b></li>
  <li>온수 쪽에서 먼저 나타나는 경우도 많음</li>
</ul>
<h2>교체 시기 판단</h2>
<p>오래된 금속관은 대략 20~30년을 넘기면 부식이 본격화됩니다. 위 신호가 겹쳐 나타나면 내시경으로 관 안쪽을 확인해 전체·부분·갱생을 정하는 순서가 중요합니다.</p>
<h2>교체 전 관리</h2>
<ol>
  <li>아침 첫 물은 1~2분 흘려보낸 뒤 사용합니다.</li>
  <li>수전 끝 거름망을 달아 이물질을 거릅니다.</li>
</ol>
""",
        "faq": [
            ("부분만 고치면 안 되나요?", "한 지점만 삭았다면 부분 수리로 충분하지만, 관 전체가 노후했다면 다른 곳이 곧 또 터집니다. 관 상태를 먼저 파악한 뒤 전체·부분·갱생 중 무엇이 맞는지 정하는 것이 비용을 아끼는 길입니다."),
            ("녹물이 잠깐 나오다 마는데 괜찮나요?", "밤새 고인 물에 녹이 섞인 초기 신호입니다. 잠깐이면 지켜봐도 되지만, 한참 틀어도 계속 붉거나 수압까지 떨어지면 부식이 진행된 것이니 점검을 권합니다."),
        ],
    },
]
